...
{
    public class ModuleTractorBeam : EngageableWeapon, IBDWeapon
    {
        #region Declarations
        ...
        public float forceTB = 100;
        public float iModifier = 5;
        public float maxRayDistance = 200000;
        public float rModifier = 0;

        [KSPField(isPersistant = true, guiActive = true, guiActiveEditor = true, guiName = "Tractor Beam Intensity"),
         UI_FloatRange(controlEnabled = true, scene = UI_Scene.All, minValue = 0.0f, maxValue = 1000.0f, stepIncrement = 5f)]
        public float forceCheck = 100;

        private float force = 100;

        private void SetTractorBeam()
        {
            var tb = this.part.FindModuleImplementing<ModuleTractorBeam>();
        ...
        public override void OnStart(StartState state)
        {
            ...
            if (hasDeployAnim)
            {
                deployState = Misc.SetUpSingleAnimation(deployAnimName, part);
            ...
            if (hasFireAnimation)
            {
                fireState = Misc.SetUpSingleAnimation(fireAnimName, part);
                ...
                for (int i = 0; i < fireTransforms.Length; i++)
                {
                    ...
                    if (Physics.Raycast(ray, out hit, maxDistance, 9076737))
                    {
                        ...
                        if (p && p.vessel && p.vessel != vessel)
                        {
                            ...
                            if (tractorBeam)
                            {
                                // Scale down force applied based on the distance to target
                                rModifier = (distance / maxRayDistance) * Convert.ToSingle(p.vessel.totalMass);
                                var averagedMass = p.vessel.totalMass / p.vessel.parts.Count;
                                var forceMultiplier = (requestResourceAmount * rModifier * iModifier);

                                if (reelIn)
                                {
                                    // Add the force to the root part of the targeted veseel ... ec draw * rangeModifier / intensityModifier = force applied
                                    p.vessel.rootPart.GetComponent<Rigidbody>().velocity = (this.part.vessel.GetWorldPos3D()
                                        - p.vessel.GetWorldPos3D()).normalized * forceMultiplier;
                                }
                                else
                                {
                                    if (tow)
                                    ...
                                    {
                                        p.vessel.GetComponent<Rigidbody>().isKinematic = true;
                                        p.vessel.SetWorldVelocity(Vector3.zero);
                                        p.vessel.GetComponent<Rigidbody>().velocity = Vector3.zero;
                                        p.vessel.GetComponent<Rigidbody>().angularVelocity = Vector3.zero;
                                        p.vessel.GetComponent<Rigidbody>().isKinematic = false;
                                    }
                                }
                            }
                            else
